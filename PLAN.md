...
- **Wire protocol:** `POST /push` to the configured desktop IP/port (this endpoint already exists in `shared/src/jvmMain/kotlin/org/example/auto2fa/Server.jvm.kt`, but currently takes plaintext `sender`/`message` form params — that shape is provisional and will change). New shape: request body is just the base64-encoded RSA ciphertext of the OTP code as raw text (`Content-Type: text/plain`); no JSON/serialization library needed for a single opaque field. Desktop responds `200 OK` on successful decrypt, and should NOT pop up a window or throw on a payload that fails to decrypt (log and drop) since anyone on the LAN who knows the IP can POST to this endpoint.
- **Android networking:** use a Ktor client (consistent with the server already using Ktor) added to `androidMain` dependencies, rather than raw `HttpURLConnection`.
...
1. **Crypto & settings foundation** — RSA keypair generation + encrypt/decrypt helpers (per the source-set strategy above); `Settings` data model with expect/actual persistence; desktop generates/persists its keypair on first launch if missing.
2. **Desktop settings UI** — replace the placeholder `App()` composable with a real settings screen showing the base64 public key and a copy button.
3. **Android settings UI** — a real settings screen with server IP/port and public-key paste fields, persisted via DataStore.
4. **Server protocol update** — change `/push` in `Server.jvm.kt` from plaintext `sender`/`message` form params to the base64-ciphertext body described above; decrypt on receipt; drop (don't crash on) payloads that fail to decrypt.
5. **Android SMS → network wiring** — `SmsReceiver` extracts the OTP code from the SMS body (a regex pulling the first run of ~4–8 digits is a reasonable first pass — flag it as a heuristic that will need per-service tuning), encrypts it with the configured public key, and POSTs it via `goAsync()` + coroutine.
6. **Desktop popup UI** — wire `onOtpReceived` to the shared state holder described above; render the popup window with the code and a working "Copy to clipboard" button.
7. **Polish (not required for MVP)** — input validation on the IP/port fields and connection-status feedback; encrypting the persisted private key at rest; TLS for the `/push` call; QR-code-based key pairing instead of manual paste; auto-dismiss/retry behavior.
