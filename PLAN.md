...
1. **Crypto & settings foundation** — Android side done: `RsaCrypto.encrypt` (RSA/OAEP-SHA256, now in `shared/src/jvmAndroidMain/kotlin/org/example/auto2fa/RsaCrypto.kt` so desktop can add `decrypt`/keygen to the same object later) and `Settings`/`SettingsRepository` (DataStore, `androidApp/src/main/kotlin/org/example/auto2fa/Settings.kt` — Android-only, since DataStore isn't available on desktop JVM; desktop will need its own settings persistence, not a shared one, since its settings shape is different — see product spec). Desktop-side keypair generation/persistence is still open.
